...
@section('container')
    @if (Auth()->user()->role === 'admin')
        <a href="/buku/tambah" class="btn btn-info mb-3">
            Tambah Buku</a>
    @endif
    <div class="card container-fluid mb-3 row">
        ...
        <div class="row row-cols-4 d-flex justify-content-start">
            ...
            @forelse ($buku as $item)
                ...
                <div class="card-container my-2 col">
                    <div class="card">
                        <div class="image-container">
                            <img src="{{ asset('storage/' . $item->gambar) }}" class="image" alt="...">
                        </div>
                        <div class="card-body">
                            <h5 class="card-title">{{ $item->judul }}</h5>
                            <p class="card-text my-2 text-truncate">Pengarang: {{ $item->pengarang }}</p>
                            <p class="card-text my-2">Kategori:
                                @foreach ($item->category_buku as $category)
                                    {{ $category->nama }}
                                @endforeach
                            </p>
                            <p class="card-text my-2">Status : {{ $item->status }}</p>
                            @if (Auth::user()->role === 'admin')
                                <div class="button-area">
                                    <button class="btn-sm btn-warning px-2"><a href="/buku/{{ $item->id }}/edit"
                                            style="text-decoration: none;color:white">Edit</a></button>
